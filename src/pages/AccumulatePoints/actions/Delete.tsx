...
import { deleteAccumulationCode } from "../../../services/accumulation";
...
function Delete(props: DeleteProps) {
  const { open, onCancel, code, setRefreshKey } = props
  ...
  const onSubmit = async () => {
    setLoading(true);
    try {
      await deleteAccumulationCode({ code })
      notification.success('Xóa mã tích điểm thành công')
      onCancel();
      ...
      if (err instanceof Error) {
        if (err.message === "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.") {
          ...
          notification.error('Xóa mã tích điểm thất bại')
        }
      }
    } finally {
      setLoading(false);
